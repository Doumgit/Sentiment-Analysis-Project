...
author: "Souleymane Doumbia, and Fomba Kassoh"
date: "2023-11-12"
...
```{r Loading the data from the Jane Austen Corpus}
...
```{r Tokenizing the text froom the books}
...
```{r Visualizing sentiment analysis}
library(ggplot2)
...
```{r Removing stop words from the text}
data(stop_words)
...
```{r Inner-jooining mpqa and tidy_brown_sentiment}
...
```{r visualization using mpqa lexicon}
library(ggplot2)
...
```{r inner_joining sentimentAnalysis and tidy_brown_sentiment }
...
```{r Visualization using SentimentAnalysis lexicon}
library(ggplot2)
...
```{r inner_joining qdap and tidy_brown_qdap_sentiment}
...
```{r visualization using lexicon qdap}
...
```{r Visualization strength sentiment}
...
```{r Visualization amplification sentiment}
